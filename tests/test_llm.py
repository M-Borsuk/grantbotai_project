...
def test_generate_section(monkeypatch, llm):
    dummy_docs = [{"id": "doc1", "section_type": "innovation_description", "text": "Test doc"}]
    ...
    monkeypatch.setattr(
        llm,
        "_OpenRouterLLM__format_contexts_for_llm__",
        lambda docs: "[INNOVATION_DESCRIPTION]\nTest doc",
    )
    monkeypatch.setattr(
        llm, "_OpenRouterLLM__system_prompt_for_section__", lambda st: "System prompt"
    )
            ...
            class Completions:
                ...
                def create(*args, **kwargs):
                    return type(
                        ...
                        {
                            "choices": [
                                type("C", (), {"message": type("M", (), {"content": "LLM Output"})})
                            ]
                        },
                    )
