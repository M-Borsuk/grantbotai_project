...
def test_generate_section_endpoint(monkeypatch):
    ...
    monkeypatch.setattr(openrouter_llm, "generate_section", lambda input_text, company_id, section_type, k=3: {
        "generated_text": "Test output", "sources": ["id1", "id2"]
    })
    payload = {
        "company_id": "123",
...
def test_history_endpoint(monkeypatch):
    ...
    monkeypatch.setattr(mongo, "history", type("C", (), {
        "find": lambda self, query: [
            {"request_id": "r1", "section_type": "innovation_description", "created_at": "now", "sources": ["id1", "id2"]}
        ],
    })())
    client = TestClient(app)
